...
export class LogToScreenErrorStreamHub implements IStreamHub {
    public OnReceiveStream(stream: RPCStream): void {
        ...
        if (
            kind === RPCStream.StreamKindSystemErrorReport ||
...
export default class Client implements IReceiver {
    ...
    private tryToTimeout(nowMS: number): void {
        ...
        for (let i = 0; i < timeoutItems.length; i++) {
            ...
            const err = ErrClientTimeout.addDebug(`${target} timeout`).addDebug(
                timeoutItems[i].stack
            );
    ...
    public tryToDeliverPreSendMessages(): void {
        ...
        while (findFree < channelSize && this.preSendHead != null) {
            // find a free channel
            while (
                findFree < channelSize &&
                this.channels[findFree].item !== null
            ) {
                findFree++;
            }
    ...
    public send(
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if (eStr !== RPCStream.StreamWriteOK) {
                item.deferred.doReject(
                    ErrUnsupportedValue.addDebug(
                        `${convertOrdinalToString(i + 1)} argument(${
                            args[i]
                        }): ${eStr}`
                    )
                );
    ...
    OnConnReadStream(streamConn: IStreamConn | null, stream: RPCStream): void {
        ...
        if (this.conn === null && streamConn !== null) {
            ...
            if (callbackID !== 0) {
                ...
                stream.getKind() !== RPCStream.StreamKindConnectResponse
            ) {
                this.OnConnError(streamConn, ErrStream);
            } else {
                ...
                if (
                    ok1 &&
                    ok2 &&
                    numOfChannels.toNumber() > 0 &&
                    ok3 &&
                    transLimit.toNumber() > 0 &&
                    ok4 &&
                    heartbeat.toNumber() > 0 &&
                    ok5 &&
                    heartbeatTimeout.toNumber() > 0 &&
                    stream.isReadFinish()
                ) {
                        ...
                        if (this.channels !== null) {
                            for (const channel of this.channels) {
                                if (channel.item != null) {
                                    this.conn.writeStream(
                                        channel.item.sendStream
                                    );
                                }
                            }
                        }
            ...
            switch (stream.getKind()) {
                case RPCStream.StreamKindRPCResponseOK:
                    if (this.channels !== null && this.channels.length > 0) {
                        const channel =
                            this.channels[callbackID % this.channels.length];
